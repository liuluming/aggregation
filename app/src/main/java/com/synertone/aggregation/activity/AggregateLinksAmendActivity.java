...
import com.synertone.aggregation.adapter.CommonAdapter;
import com.synertone.aggregation.adapter.CommonViewHolder;
import com.synertone.aggregation.adapter.SpinnerAdapter;
import com.synertone.aggregation.bean.AggregatedLinkInfoModel;
...
public class AggregateLinksAmendActivity extends BaseActivity {
    ...
    private void doAggregatedLinkConfig() {
        try {
            ...
            if (currentLinkinfo == null) {
                ...
                if(choosedChannels.size()>1) {
                    linkBeanAdd.setChannelList(choosedChannels);
                }else{
                    Toast.makeText(mContext, "请选择两个或两个以上的通道！", Toast.LENGTH_LONG).show();
                    return;
                }
    ...
    private boolean verifyInput(List<AggregatedLinkInfoModel.LinkinfoBean.ChannelListBean> choosedChannels) {
        ...
        if (StringUtils.isEmpty(s)) {
            ...
            return true;
        }else if(!StringUtils.isEmpty(s)&&s.length()>64){
