...
import com.synertone.aggregation.adapter.SpinnerAdapter;
import com.synertone.aggregation.bean.APNModel;
import com.synertone.aggregation.bean.AdapterInfoModel;
import com.synertone.aggregation.bean.AdapterModel;
import com.synertone.aggregation.bean.BaseModel;
import com.synertone.aggregation.bean.InterfaceModel;
import com.synertone.aggregation.bean.PPPoEModel;
import com.synertone.aggregation.bean.StaticIPModel;
...
import com.synertone.aggregation.utils.TransformUtils;
    ...
    private void initEvent() {
        btSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (StringUtils.isEmpty(etNetAdapterName.getText().toString())) {
                    Toast.makeText(mContext, "请输入适配器名称", Toast.LENGTH_LONG).show();
                    return;
                }else if(!StringUtils.isEmpty(etNetAdapterName.getText().toString())&&(etNetAdapterName.getText().toString().length()>64)){
                    Toast.makeText(mContext, "适配器名称最多输入64字符，请重新输入！", Toast.LENGTH_LONG).show();
                    return;
                }
